...
// ── Time-series bucketing (day / week / month) ───────────

const MONTHS = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
const pad2 = (n: number) => String(n).padStart(2, "0");

type BucketKey = { key: string; date: string; label: string };

function dayKey(d: Date): BucketKey {
  const date = `${d.getUTCFullYear()}-${pad2(d.getUTCMonth() + 1)}-${pad2(d.getUTCDate())}`;
  return { key: date, date, label: `${MONTHS[d.getUTCMonth()]} ${d.getUTCDate()}` };
}

function weekKey(d: Date): BucketKey {
  // Monday-start week
  const t = new Date(Date.UTC(d.getUTCFullYear(), d.getUTCMonth(), d.getUTCDate()));
  const dow = (t.getUTCDay() + 6) % 7; // 0 = Monday
  t.setUTCDate(t.getUTCDate() - dow);
  const date = `${t.getUTCFullYear()}-${pad2(t.getUTCMonth() + 1)}-${pad2(t.getUTCDate())}`;
  return { key: date, date, label: `${MONTHS[t.getUTCMonth()]} ${t.getUTCDate()}` };
}

function monthKey(d: Date): BucketKey {
  const ym = `${d.getUTCFullYear()}-${pad2(d.getUTCMonth() + 1)}`;
  return { key: ym, date: `${ym}-01`, label: `${MONTHS[d.getUTCMonth()]} ${d.getUTCFullYear()}` };
}

function buildSeries(
  events: Record<string, unknown>[],
  keyFn: (d: Date) => BucketKey
): TrafficPoint[] {
  type Acc = Omit<TrafficPoint, "visitors"> & { ips: Set<string> };
  const map = new Map<string, Acc>();
  for (const e of events) {
    const d = new Date(e.created_at as string);
    const { key, date, label } = keyFn(d);
    let b = map.get(key);
    if (!b) {
      b = { date, label, events: 0, programViews: 0, clicks: 0, searches: 0, ips: new Set() };
      map.set(key, b);
    }
    b.events++;
    if (e.ip_hash) b.ips.add(e.ip_hash as string);
    if (e.type === "program_view") b.programViews++;
    else if (e.type === "outbound_click") b.clicks++;
    else if (e.type === "search") b.searches++;
  }
  return Array.from(map.values())
    .sort((a, b) => a.date.localeCompare(b.date))
    .map(({ ips, ...rest }) => ({ ...rest, visitors: ips.size }));
}
...
export default async function AdminPage({
  ...
  // ── Traffic time-series (day / week / month, full 60d window) ──
  const trafficSeries = {
    day: buildSeries(events60d, dayKey),
    week: buildSeries(events60d, weekKey),
    month: buildSeries(events60d, monthKey),
  };
  ...
  // ── Content Lab ──
  const clEvents = events30d.filter((e) => e.type === "content_lab_generate");
  const clEventsToday = eventsToday.filter((e) => e.type === "content_lab_generate");
  const clEvents7d = events7d.filter((e) => e.type === "content_lab_generate");
  const clUniqueUsers = new Set(clEvents.map((e) => e.ip_hash)).size;

  const clModelMap = new Map<string, number>();
  const clPlatformMap = new Map<string, number>();
  const clFormulaMap = new Map<string, number>();
  const clProgramMap = new Map<string, number>();
  const clLangMap = new Map<string, number>();
  let clTotalTokens = 0;

  for (const e of clEvents) {
    const m = e.metadata as Record<string, unknown> | undefined;
    if (!m) continue;
    if (m.model) clModelMap.set(m.model as string, (clModelMap.get(m.model as string) ?? 0) + 1);
    if (m.platform) clPlatformMap.set(m.platform as string, (clPlatformMap.get(m.platform as string) ?? 0) + 1);
    if (m.formula) clFormulaMap.set(m.formula as string, (clFormulaMap.get(m.formula as string) ?? 0) + 1);
    if (m.language) clLangMap.set(m.language as string, (clLangMap.get(m.language as string) ?? 0) + 1);
    if (m.tokens) clTotalTokens += m.tokens as number;
    const slugs = m.programs as string[] | undefined;
    if (slugs) {
      for (const s of slugs) {
        clProgramMap.set(s, (clProgramMap.get(s) ?? 0) + 1);
      }
    }
  }

  const clTopModels = Array.from(clModelMap.entries()).sort((a, b) => b[1] - a[1]);
  const clTopPlatforms = Array.from(clPlatformMap.entries()).sort((a, b) => b[1] - a[1]);
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <div className="mx-auto max-w-7xl px-6 py-8">
        ...
        {/* ── Content Lab ── */}
        <Section title="Content Lab" subtitle="AI content generation usage">
          {clEvents.length === 0 ? (
            <Empty />
          ) : (
            <>
              ...
              <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-6">
                <div className="rounded-lg border border-border/30 bg-card/30 p-3 text-center">
                  <div className="text-lg font-bold tabular-nums">{clEvents.length}</div>
                  <div className="text-[10px] text-muted-foreground mt-0.5">Generations (30d)</div>
                </div>
                <div className="rounded-lg border border-border/30 bg-card/30 p-3 text-center">
                  <div className="text-lg font-bold tabular-nums">{clEventsToday.length}</div>
                  <div className="text-[10px] text-muted-foreground mt-0.5">Today</div>
                </div>
                <div className="rounded-lg border border-border/30 bg-card/30 p-3 text-center">
                  <div className="text-lg font-bold tabular-nums">{clEvents7d.length}</div>
                  <div className="text-[10px] text-muted-foreground mt-0.5">7d</div>
                </div>
                <div className="rounded-lg border border-border/30 bg-card/30 p-3 text-center">
                  <div className="text-lg font-bold tabular-nums">{clUniqueUsers}</div>
                  <div className="text-[10px] text-muted-foreground mt-0.5">Unique Users</div>
                </div>
                <div className="rounded-lg border border-border/30 bg-card/30 p-3 text-center">
                  <div className="text-lg font-bold tabular-nums">{clTotalTokens.toLocaleString()}</div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                {/* Models */}
                <div>
                  <p className="text-[10px] uppercase tracking-wide text-muted-foreground/60 font-medium mb-2">Models</p>
                  <div className="space-y-1.5">
                    {clTopModels.map(([m, count]) => (
                ...
                {/* Platforms */}
                <div>
                  <p className="text-[10px] uppercase tracking-wide text-muted-foreground/60 font-medium mb-2">Platforms</p>
                  <div className="space-y-1.5">
                    {clTopPlatforms.map(([p, count]) => (
                ...
                {/* Formulas */}
                <div>
                  <p className="text-[10px] uppercase tracking-wide text-muted-foreground/60 font-medium mb-2">Formulas</p>
                  <div className="space-y-1.5">
                    {clTopFormulas.map(([f, count]) => (
                      <div key={f} className="flex items-center justify-between text-xs">
                        <span className="uppercase text-[11px]">{f}</span>
                        <span className="text-muted-foreground tabular-nums ml-2">{count}</span>
                      </div>
                    ))}
                  </div>
                ...
                {/* Languages */}
                <div>
                  <p className="text-[10px] uppercase tracking-wide text-muted-foreground/60 font-medium mb-2">Languages</p>
                  <div className="space-y-1.5">
                    {clTopLangs.map(([l, count]) => (
              ...
              {/* Top Programs */}
              {clTopPrograms.length > 0 && (
                <div className="mt-6 pt-4 border-t border-border/30">
                  <p className="text-[10px] uppercase tracking-wide text-muted-foreground/60 font-medium mb-2">
                    Top Programs Generated
                  </p>
                  <div className="flex flex-wrap gap-1.5">
                    {clTopPrograms.map(([slug, count]) => (
                      <span key={slug} className="rounded-md border border-emerald-500/20 bg-emerald-500/5 px-2 py-0.5 text-[11px] text-emerald-400">
                        {slug} <span className="text-muted-foreground">({count})</span>
                      </span>
                    ))}
                  </div>
                </div>
...
function EventDot({ type }: { type: string }) {
  const colors: Record<string, string> = {
    ...
    compare: "bg-cyan-500",
    content_lab_generate: "bg-yellow-500",
  };
